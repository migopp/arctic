use core::convert::Infallible;
use core::marker::PhantomData;
use core::mem;
use core::sync::atomic::Ordering;

use ribbit::Unpack as _;

use crate::edge;
use crate::key;
use crate::node;
use crate::node::Frozen;
use crate::node::Node3;
use crate::Edge;

pub(crate) struct Cursor<'a, 'k, P> {
    key: &'k [u8],
    index: usize,
    here: &'a Edge,
    history: P,
}

#[derive(Debug)]
pub(crate) enum Op {
    Node(node::Op),
    Edge(edge::Op),
}

impl<'a, 'k, P: History<'a>> Cursor<'a, 'k, P> {
    pub(crate) fn new(root: &'a Edge, key: &'k [u8]) -> Self {
        Self {
            key,
            index: 0,
            here: root,
            history: P::default(),
        }
    }

    pub(crate) fn traverse_weak(&mut self) -> Option<(edge::Meta, edge::Data)> {
        loop {
            let edge = self.here();
            let meta_packed = edge.load_low_packed(Ordering::Relaxed);
            let meta = meta_packed.unpack();

            let child = meta.child()?;
            let key = self.key();
            let len = meta.match_weak(key)?;

            let data = edge.load_high(Ordering::Acquire);

            // Validate metadata consistency
            if edge.load_low_packed(Ordering::Relaxed) != meta_packed {
                continue;
            }

            let node = match child {
                edge::Child::Leaf { removed: _ } => return Some((meta, data)),
                edge::Child::Node(kind) => unsafe { data.to_node(kind) },
            };
            let byte = key.get(len.to_usize())?;
            let next = unsafe { node.as_node() }.get(*byte)?;
            self.push(len, node, next);
        }
    }

    pub(crate) fn traverse_strong(
        &mut self,
        value: u64,
    ) -> (Op, (edge::Meta, edge::Data), (edge::Meta, edge::Data)) {
        loop {
            let edge = self.here();
            let key = self.key();

            let old_meta_packed = edge.load_low_packed(Ordering::Relaxed);
            let old_meta = old_meta_packed.unpack();

            // Uninit edge doesn't require key match
            let Some(child) = old_meta.child() else {
                let (op, new_meta, new_data) = match key.len() > key::Len::MAX {
                    true => (
                        Op::Edge(edge::Op::Create),
                        edge::Meta {
                            key: key::Array::from_slice(&key[..key::Len::MAX]),
                            frozen: false,
                            kind: node::Kind::Node3,
                        },
                        edge::Data::new_node::<Node3, _>(core::iter::empty()),
                    ),
                    false => (
                        Op::Edge(edge::Op::Insert),
                        edge::Meta {
                            key: key::Array::from_slice(key),
                            frozen: false,
                            kind: node::Kind::Leaf,
                        },
                        edge::Data::new_leaf(value),
                    ),
                };
                return (op, (old_meta, edge::Data::default()), (new_meta, new_data));
            };

            let old_data = edge.load_high(Ordering::Relaxed);
            let key = self.key();

            let prefix = match old_meta.match_strong(key) {
                edge::Match::Full(prefix) => prefix,
                edge::Match::Partial { start, middle, end } => {
                    return (
                        Op::Edge(edge::Op::Expand),
                        (old_meta, old_data),
                        (
                            edge::Meta {
                                key: start,
                                frozen: false,
                                kind: node::Kind::Node3,
                            },
                            edge::Data::new_node::<Node3, _>(core::iter::once((
                                middle,
                                edge::Meta {
                                    key: end,
                                    frozen: false,
                                    kind: old_meta.kind,
                                },
                                old_data,
                            ))),
                        ),
                    )
                }
            };

            let kind = match child {
                edge::Child::Node(kind) => kind,
                edge::Child::Leaf { removed: _ } => {
                    return (
                        Op::Edge(edge::Op::Insert),
                        (old_meta, old_data),
                        (
                            edge::Meta {
                                key: key::Array::from_slice(key),
                                frozen: false,
                                kind: node::Kind::Leaf,
                            },
                            edge::Data::new_leaf(value),
                        ),
                    )
                }
            };

            // Validate metadata consistency before accessing node
            if edge.load_low_packed(Ordering::Relaxed) != old_meta_packed {
                continue;
            }

            let node_ref = unsafe { old_data.to_node(kind) };
            let node = unsafe { node_ref.as_node() };

            match self.history.freeze() {
                true if node.is_frozen() => (),
                true | false => {
                    let byte = key[prefix.to_usize()];
                    #[allow(clippy::single_match)]
                    match node.get_or_reserve(byte) {
                        // Fast path: no need to replace
                        Ok(edge) => {
                            self.push(prefix, node_ref, edge);
                            continue;
                        }
                        Err(Frozen) => (),
                    }
                }
            };

            node.freeze();
            let (op, new_meta, new_data) = node.replace(&old_meta);
            return (Op::Node(op), (old_meta, old_data), (new_meta, new_data));
        }
    }

    fn push(&mut self, len: key::Len, node: node::Ref, edge: &'a Edge) {
        self.index += len.to_usize();
        self.index += 1;
        self.history.push(Segment {
            len,
            edge: self.here,
            node,
        });
        self.here = edge;
    }

    pub(crate) fn pop(&mut self) -> Result<node::Ref, P::PopError> {
        let segment = self.history.pop()?.expect("Root edge can never be frozen");
        self.index -= 1;
        self.index -= segment.len.to_usize();
        self.here = segment.edge;
        Ok(segment.node)
    }

    #[inline]
    pub(crate) fn here(&self) -> &Edge {
        self.here
    }

    #[inline]
    pub(crate) fn key(&self) -> &'k [u8] {
        &self.key[self.index..]
    }
}

pub(crate) trait History<'a>: Default {
    type PopError;
    fn freeze(&mut self) -> bool;
    fn push(&mut self, segment: Segment<'a>);
    fn pop(&mut self) -> Result<Option<Segment<'a>>, Self::PopError>;
}

#[derive(Default)]
pub(crate) struct Optimistic<'a>(PhantomData<&'a ()>);

impl<'a> History<'a> for Optimistic<'a> {
    type PopError = ();

    #[inline]
    fn freeze(&mut self) -> bool {
        false
    }

    #[inline]
    fn push(&mut self, _segment: Segment<'a>) {}

    #[inline]
    fn pop(&mut self) -> Result<Option<Segment<'a>>, Self::PopError> {
        Err(())
    }
}

#[derive(Default)]
pub(crate) struct Pessimistic<'a> {
    freeze: bool,
    path: Vec<Segment<'a>>,
}

impl<'a> History<'a> for Pessimistic<'a> {
    type PopError = Infallible;

    #[inline]
    fn freeze(&mut self) -> bool {
        mem::take(&mut self.freeze)
    }

    #[inline]
    fn push(&mut self, segment: Segment<'a>) {
        self.path.push(segment)
    }

    #[inline]
    fn pop(&mut self) -> Result<Option<Segment<'a>>, Self::PopError> {
        self.freeze = true;
        Ok(self.path.pop())
    }
}

#[derive(Debug)]
pub(crate) struct Segment<'a> {
    len: key::Len,
    edge: &'a Edge,
    node: node::Ref,
}
